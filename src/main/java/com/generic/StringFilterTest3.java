...
//        for (Map.Entry<String, String> entry: getAsProperies(Headers1, allowedHeaders).entrySet()) System.out.println(entry.getKey() + " = " + entry.getValue());
...
//        for (Map.Entry<String, String> entry: getAsProperies("Headers", allowedHeaders).entrySet()) System.out.println(entry.getKey() + " = " + entry.getValue());
    }

    static Map<String,String> getAsProperies(String value, String[] allowedHeaders) {
        try {
            return getAsProperies1(value, allowedHeaders);
        } catch (Exception e1) {
            try {
                return getAsProperies2(value, allowedHeaders);
            } catch (Exception e2) {}
        }
        return new HashMap<>();
    }
